#ifndef DBGDRAW_OPENGL33_H
#define DBGDRAW_OPENGL33_H
...
dd_backend_init(dd_ctx_t *ctx)
{
  ...
  GLCHECK(glGenBuffers(1, &backend.vbo));
  GLCHECK(glGenBuffers(1, &backend.ibo));
  ...
  glGenTextures(1, &backend.line_data_texture_id);
  glBindTexture(GL_TEXTURE_BUFFER, backend.line_data_texture_id);
  glTexBuffer(GL_TEXTURE_BUFFER, GL_RGBA32F, backend.vbo);
...
dd_backend_render(dd_ctx_t *ctx)
{
  ...
  GLCHECK(glBindBuffer(GL_ARRAY_BUFFER, ctx->backend->vbo));
  if (ctx->backend->vbo_size < ctx->verts_cap * sizeof(dd_vertex_t))
  {
    ctx->backend->vbo_size = ctx->verts_cap * sizeof(dd_vertex_t);
    GLCHECK(glBufferData(GL_ARRAY_BUFFER, ctx->backend->vbo_size, NULL, GL_DYNAMIC_DRAW));
  }
  ...
  for (int32_t i = 0; i < ctx->commands_len; ++i)
  {
    ...
    {
      GLCHECK(glBindBuffer(GL_ARRAY_BUFFER, ctx->backend->ibo));
      if (ctx->backend->ibo_size < ctx->instance_cap * sizeof(dd_instance_data_t))
      {
        ...
        GLCHECK(glBufferData(GL_ARRAY_BUFFER, ctx->backend->ibo_size, NULL, GL_DYNAMIC_DRAW));
      }
      GLCHECK(glBufferSubData(GL_ARRAY_BUFFER, 0, ctx->backend->ibo_size, cmd->instance_data));
      GLCHECK(glBindBuffer(GL_ARRAY_BUFFER, 0));
    }
...
dd_backend_init_font_texture(dd_ctx_t *ctx, const uint8_t *data, int32_t width, int32_t height,
                             uint32_t *tex_id)
{
  GLCHECK(glGenTextures(1, &ctx->backend->font_tex_ids[ctx->fonts_len]));
  GLCHECK(glBindTexture(GL_TEXTURE_2D, ctx->backend->font_tex_ids[ctx->fonts_len]));
