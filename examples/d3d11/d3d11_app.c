...
d3d11_init(d3d11_ctx_t *d3d11, const d3d11_ctx_desc_t *desc)
{
  ...
  for (int32_t i = 0; i < win_title_len; ++i)
  {
    win_title_wide[i] = desc->win_title[i];
  }
...
#if defined(D3D11_APP_ENABLE_DEBUG)
...
d3d11_window_procedure(HWND window_handle, UINT message, WPARAM w_param, LPARAM l_param)
{
  ...
  {
    mods |= D3D11_APP_MODIFIER_SHIFT;
  }
  if (GetKeyState(VK_CONTROL) & (1 << 31))
  {
    mods |= D3D11_APP_MODIFIER_CTRL;
  }
  if (GetKeyState(VK_MENU) & (1 << 31))
  {
    mods |= D3D11_APP_MODIFIER_ALT;
  }
  if (GetKeyState(VK_LWIN) & (1 << 31))
  {
    mods |= D3D11_APP_MODIFIER_SUPER;
  }
  if (GetKeyState(VK_RWIN) & (1 << 31))
  {
    mods |= D3D11_APP_MODIFIER_SUPER;
  }
...
void d3d11_create_default_render_target(d3d11_ctx_t *d3d11)
{
  HRESULT hr;
...
void d3d11_destroy_default_render_target(d3d11_ctx_t *d3d11)
{
  assert(d3d11);
  if (d3d11->render_target)
  {
    ID3D11Texture2D_Release(d3d11->render_target);
    d3d11->render_target = NULL;
  }
  if (d3d11->depth_stencil)
  {
    ID3D11Texture2D_Release(d3d11->depth_stencil);
    d3d11->depth_stencil = NULL;
  }
  if (d3d11->render_target_view)
  {
    ID3D11RenderTargetView_Release(d3d11->render_target_view);
    d3d11->render_target_view = NULL;
  }
  if (d3d11->depth_stencil_view)
  {
    ID3D11DepthStencilView_Release(d3d11->depth_stencil_view);
    d3d11->depth_stencil_view = NULL;
  }
}

void d3d11_update_default_render_target(d3d11_ctx_t *d3d11)
{
  assert(d3d11);
  if (d3d11->swap_chain)
  {
    ...
    IDXGISwapChain_ResizeBuffers(d3d11->swap_chain, 1, w, h, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
    d3d11_create_default_render_target(d3d11);
  }
}
...
void _d3d11_init_keycode_table(void)
{
  if (_d3d11_keycode_map_initialized)
  {
    return;
  }
