...
init(app_state_t* state)
{
  ...
  d3d11_ctx_desc_t d3d11_desc = {.win_title    = "dbgdraw_d3d11_primitives",
                                 .win_x        = 100,
                                 .win_y        = 100,
                                 .win_w        = 640,
                                 .win_h        = 320,
  ...
  dd_render_backend_t* primitives_backend =
    calloc(1, sizeof(dd_render_backend_t));
  ...
  dd_ctx_desc_t desc_overlay = {.max_vertices        = 1024,
                                .max_commands        = 16,
                                .detail_level        = 2,
  ...
  msh_camera_init(
    ...
    &(msh_camera_desc_t) {
      ...
      .up     = msh_vec3_posy(),
      .viewport =
        msh_vec4(0, 0, (float)d3d11_desc.win_w, (float)d3d11_desc.win_h),
...
frame(app_state_t* state)
{
  ...
  msh_vec2_t scrn_p0 =
    msh_vec2((float)input->mouse.prev_x, (float)input->mouse.prev_y);
  msh_vec2_t scrn_p1 =
    msh_vec2((float)input->mouse.cur_x, (float)input->mouse.cur_y);
  ...
  if (input->keys[D3D11_APP_KEYCODE_1])
  {
    show_points                      = !show_points;
    input->keys[D3D11_APP_KEYCODE_1] = 0;
  }
  if (input->keys[D3D11_APP_KEYCODE_2])
  {
    show_lines                       = !show_lines;
    input->keys[D3D11_APP_KEYCODE_2] = 0;
  }
  if (input->keys[D3D11_APP_KEYCODE_3])
  {
    show_solid                       = !show_solid;
    input->keys[D3D11_APP_KEYCODE_3] = 0;
  }
  if (input->keys[D3D11_APP_KEYCODE_4])
  {
    shading_mode                     = !shading_mode;
    input->keys[D3D11_APP_KEYCODE_4] = 0;
  }
  if (input->keys[D3D11_APP_KEYCODE_5])
  {
    show_overlay                     = !show_overlay;
    input->keys[D3D11_APP_KEYCODE_5] = 0;
  }

  if (input->keys[D3D11_APP_KEYCODE_EQUAL])
  {
    detail_lvl++;
    input->keys[D3D11_APP_KEYCODE_EQUAL] = 0;
  }
  if (input->keys[D3D11_APP_KEYCODE_MINUS])
  {
    detail_lvl--;
    input->keys[D3D11_APP_KEYCODE_MINUS] = 0;
  }
