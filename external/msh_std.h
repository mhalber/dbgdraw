/*
  ...
  be included, define: #define MSH_STD_INCLUDE_LIBC_HEADERS
  ...
  [ ] Custom prints (stb_sprintf inlined, look at replacing sprintf with "write"
  function in linux (unistd.h))
...
#define MSH_PLATFORM_NAME                                                      \
  "asm.js " MSH_STRINGIZE(__EMSCRIPTEN_major__) "." MSH_STRINGIZE(             \
    __EMSCRIPTEN_minor__) "." MSH_STRINGIZE(__EMSCRIPTEN_tiny__)
...
#define MSH_COMPILER_NAME                                                      \
  "GCC " MSH_STRINGIZE(__GNUC__) "." MSH_STRINGIZE(                            \
    __GNUC_MINOR__) "." MSH_STRINGIZE(__GNUC_PATCHLEVEL__)
#elif MSH_COMPILER_CLANG
#define MSH_COMPILER_NAME                                                      \
  "Clang " MSH_STRINGIZE(__clang_major__) "." MSH_STRINGIZE(                   \
    __clang_minor__) "." MSH_STRINGIZE(__clang_patchlevel__)
...
MSHDEF void msh_print_progress_bar(char* prefix,
                                   char* suffix,
                                   uint64_t iter,
                                   uint64_t total,
                                   int32_t len);
...
// Look at Niklas Frykholm stuff on allocators to get some idea on when and what
// allocators are relevant Maybe start with simple stack allocator --> it is
// going to be similar to msh_array
...
MSHDEF size_t msh_strcpy_range(char* dst,
                               const char* src,
                               size_t start,
                               size_t len);
...
#define msh_accumulate(x, n)                                                   \
  _Generic((x), int32_t                                                        \
           : msh_accumulatei, float                                            \
           : msh_accumulatef, double                                           \
           : msh_accumulated, default                                          \
           : msh_accumulatef)(x, n)
#endif
...
MSHDEF void msh_invert_cdf(const double* cdf,
                           int32_t n_vals,
                           double* invcdf,
...
// Current issues:
// - If function is hit multipe times, how to store timing? -- Just like
// handmade hero, record events linearly, and store to which record they hark
// back, then print record information after pairing the events.
...
// // ((uint64_t)__FUNCTION__ % 15487249) + ((uint64_t)__FILE__ % 1300613) +
// __LINE__; #define MSH_BEGIN_TIMED_BLOCK() msh_debug_begin_timed_block( uid,
// (char*)__FILE__, (char*)__FUNCTION__, __LINE__ ) #define
// MSH_END_TIMED_BLOCK() msh_debug_end_timed_block( uid, (char*)__FILE__,
// (char*)__FUNCTION__, __LINE__ )

// MSHDEF void
// msh_debug_begin_timed_block( int32_t counter, char* filename, char*
// function_name, int32_t line_number )
// {
...
// MSHDEF void
// msh_debug_end_timed_block( int32_t counter, char* filename, char*
// function_name, int32_t line_number )
// {
...
// {
//   for( size_t event_idx = 0; event_idx < msh_array_len( DEBUG_EVENT_ARRAY );
//   ++event_idx )
//   {
...
//     {
//       printf("ERROR! Block starting at line %d in %s(function %s) does not
//       have pairing end block\n",
//              start_event->line_number, start_event->filename,
//              start_event->function_name );
//     }
//     else
//     {
...
//       end_line, start_event->function_name, start_event->filename,
//       (int32_t)n_cycles ); start_event->type = MSH_DEBUG_EVENT_PROCESSED;
